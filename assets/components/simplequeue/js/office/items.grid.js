...
Ext.extend(simpleQueue.grid.Items, OfficeExt.grid.Default, {
	windows: {},
	...
	removeItem: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('simplequeue_items_remove')
				: _('simplequeue_item_remove'),
			text: ids.length > 1
				? _('simplequeue_items_remove_confirm')
				: _('simplequeue_item_remove_confirm'),
			url: this.config.url,
			params: {
				action: 'simplequeue/processor',
				method: 'item/remove',
	...
	getColumns: function (config) {
		return [{
			header: _('simplequeue_item_id'),
			dataIndex: 'id',
			sortable: true,
			width: 70
		}, {
			header: _('simplequeue_item_name'),
			dataIndex: 'name',
			sortable: true,
			width: 200,
		}, {
			header: _('simplequeue_item_description'),
			dataIndex: 'description',
			sortable: false,
			width: 250,
		}, {
			header: _('simplequeue_item_active'),
			dataIndex: 'active',
	...
	getTopBar: function (config) {
		return [{
			text: '<i class="fa fa-plus"></i>&nbsp;' + _('simplequeue_item_create'),
